    ...
    <div class="configuracionAlarma">
        ...
        <div class="divTiempoLabel">
            <div class="labelInput">
                ...
                <input type="number" id="minutos1" placeholder="Minutos" min="0" max="59" @input="validarMinutos(1)" />
            ...
            <div class="labelInput">
                ...
                <input type="number" id="minutos2" placeholder="Minutos" min="0" max="59" @input="validarMinutos(2)" />
        ...
        <div class="botones">
            ...
            <button v-if="temporizadorStore.temporizadores[temporizadorActivo - 1].estado === 'iniciado'"
                @click="pausarTemporizador(temporizadorActivo)" class="btn btn-pausar">
                PAUSAR
            </button>

            <button v-if="temporizadorStore.temporizadores[temporizadorActivo - 1].estado === 'pausado'"
                @click="pausarTemporizador(temporizadorActivo)" class="btn btn-pausar">
                REANUDAR
            </button>
...
export default {
    setup() {
        ...
        const temporizadorActivo = ref(1);
        ...
        const validarMinutos = (temporizadorId) => {
            const minutosInput = document.getElementById(`minutos${temporizadorId}`).value;
        ...
        const validarSegundos = (temporizadorId) => {
            const segundosInput = document.getElementById(`segundos${temporizadorId}`).value;
        ...
        const iniciarTemporizador = (temporizadorId) => {
            const minutos = parseInt(document.getElementById(`minutos${temporizadorId}`).value) || 0;
            const segundos = parseInt(document.getElementById(`segundos${temporizadorId}`).value) || 0;
            temporizadorStore.iniciar(temporizadorId, minutos, segundos);
            temporizadorActivo.value = temporizadorId;
            const siguienteTemporizadorId = temporizadorId === 1 ? 2 : 1;

            const intervaloEspera = setInterval(() => {
                const temporizador = temporizadorStore.temporizadores[temporizadorId - 1];
        ...
        const reiniciarTemporizador = (temporizadorId) => {
            const minutos = parseInt(document.getElementById(`minutos${temporizadorId}`).value) || 0;
            const segundos = parseInt(document.getElementById(`segundos${temporizadorId}`).value) || 0;
            temporizadorStore.reiniciar(temporizadorId, minutos, segundos);
            temporizadorActivo.value = temporizadorId;
        };
